...
Permanent record of decisions made in spec gaps that weren't promoted to the design docs. Future Claude Code sessions reference these when touching adjacent code.
...
**p1m01 — Project scaffold + game states** (S)
...
**p1m02 — Config + constants + log + registry** (S)
...
**p1m06 — Unit spawning + rendering** (S)
...
`simulation/resources.lua` module. Stack entity (id, type, amount). Tile inventory container on stockpiles (one tile_entry per tile, contents array; container-level type-keyed `reserved_in`/`reserved_out`). Public API: query verbs (`getStock`, `getAvailableStock`, `getAvailableCapacity`, `accepts`, `countWeight`), atomic transfer verbs (`produceInto`, `produceIntoCarrying`, `consumeFrom`, `moveToCarrying`, `moveFromCarrying`, `transfer`, `equipFromContainer`), reservation verbs (`reserveInbound`, `reserveOutbound`, `releaseInbound`, `releaseOutbound`), filter verbs (`setFilterMode`, `setFilterLimit`, `setFilterSource`), count verbs (`rebuildCounts`, `validateCounts`). File-local primitives (`create`, `destroy`, `deposit`, `withdraw`, `carryEntity`, `withdrawFromCarrying`, `equip`, `unequip`) — not exposed. `world.resource_counts` with category tallies (storage, carrying, ground for P1).
...
Verify: Write tests — `produceInto` wood into stockpile tile, `moveToCarrying` a partial amount (splits stack), capacity enforcement, reservation arithmetic (available = stock - reserved_out), `rebuildCounts` matches running tallies. Manually spawn resources via debug command → see them in stockpile debug dump.
...
Designation interaction mode: A enters chop designation mode. Click-drag to designate tiles. Each designated tree tile gets a `tile.designation = "chop"` marker and a posted chop activity. Cancel designation mode (X) removes designations and their activities. Visual indicator on designated tiles. Chop work cycle: unit claims chop activity → paths adjacent-to-rect (1×1) to tree → work action (duration from PlantConfig) → tree removed (`plant_type = nil`, `plant_growth = 0`) → wood granted to `unit.carrying` via `resources.produceIntoCarrying()` → check for more designated trees within reach → if carrying full or no more trees, self-deposit to nearest stockpile. Designation consumed on completion.
Docs: BEHAVIOR.md § Gathering Work Cycle (designation variant). WORLD.md § Plant System (harvest). TABLES.md § PlantConfig (tree), ActivityTypeConfig (woodcutter). UI.md § Designation buttons. ECONOMY.md § Resources Module (produceIntoCarrying).
...
**p1m19 — Cottage + home assignment** (S)
Cottage building (3×3, tile map with indoor and door tiles, 4 beds). Placement mode (temporary debug key). Instant placement in P1. Tile map rendering (indoor tiles as a lighter interior area, building outline drawn as walls between indoor and outdoor tiles, door rendered as a gap in the outline). Home assignment: when a unit needs a home, assign first housing building with available bed. `unit.home_id` ↔ `building.housing.member_ids`, `unit.bed_index` ↔ `bed.unit_id`. Trigger home assignment on building completion and when units are homeless. Sleep destination changes: if `home_id` set, travel home to sleep. If homeless, sleep on current tile (existing behavior). Housing bins initialized from HousingBinConfig (food storage at home — used by eating in p1m23).
Docs: WORLD.md § Building Layout (tile types, tile map, clearing, orientation). BEHAVIOR.md § Home Assignment, Sleep (destination). TABLES.md § BuildingConfig (cottage), HousingBinConfig, building.housing.
